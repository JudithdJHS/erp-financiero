...
def create_factura(db: Session, factura: schemas.FacturaCreate):
    ...
    if not presupuesto:
        # Buscamos nombres para dar un error más humano
        cat_nombre = db.query(models.Categoria.nombre).filter(models.Categoria.id == factura.categoria_id).scalar() or f"ID {factura.categoria_id}"
        camp_nombre = db.query(models.CampanaEvento.nombre).filter(models.CampanaEvento.id == factura.campana_evento_id).scalar() or f"ID {factura.campana_evento_id}"
        
        raise ValueError(f"No existe un presupuesto configurado para la categoría '{cat_nombre}' en la campaña '{camp_nombre}'. Debe crearlo primero en el módulo de Presupuestos.")


    # Preparamos los datos para la inserción eliminando categoria_id (que es solo para validación)
    factura_data = factura.model_dump()
    factura_data.pop("categoria_id", None)

    db_factura = models.Factura(**factura_data)
    db.add(db_factura)

    # Actualizamos el monto ejecutado de forma robusta (Decimal + Decimal)
    actual = Decimal(str(presupuesto.monto_ejecutado or 0))
    nuevo = Decimal(str(factura.monto or 0))
    presupuesto.monto_ejecutado = actual + nuevo
