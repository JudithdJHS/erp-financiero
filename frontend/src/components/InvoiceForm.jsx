...
const InvoiceForm = ({ onGastoRegistrado }) => {
  ...
  const handleOCR = async (selectedFile) => {
    ...
    try {
      ...
      const data = res.data;
      const isSimulation = data.ocr_payload?.mode === 'simulation';
      ...
      if (data.error) {
        ...
        if (isSimulation) {
          setErrorMsg("💡 Nota: El sistema está en MODO SIMULACIÓN (Google Cloud no configurado). Los datos mostrados son de ejemplo.");
        }
      }
    } catch (error) {
      console.error("OCR Error", error);
      setErrorMsg("Error conectando al servicio de análisis de facturas. Asegúrate de que el backend esté arriba.");
    } finally {
      setOcrLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.presupuesto_id) {
      setErrorMsg("🔴 Acción Bloqueada: No existe un presupuesto configurado para esta combinación de Campaña y Categoría. Por favor, créalo primero.");
      return;
    }
    ...
    try {
      await api.post('/invoices/', {
        ...
        categoria_id: parseInt(formData.categoria_id), // Añadido para validación en backend
      ...
      console.error(error);
      setErrorMsg(error.response?.data?.detail || "Error al guardar el gasto comercial.");
    } finally {
      setSaving(false);
    }
  };
